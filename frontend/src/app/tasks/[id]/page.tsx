...
interface Clip {
  ...
  created_at: string;
  video_url: string;
  // Virality scores (legacy 0-25 scale)
  ...
  // Viral scorecard (0-10 subscores + 0-100 overall) and post metadata
  ...
  payoff_score?: number;
  loop_score?: number;
  standalone_context_score?: number;
...
export default function TaskPage() {
  ...
  const [projectFontFamily, setProjectFontFamily] = useState("THEBOLDFONT");
  const [projectFontSize, setProjectFontSize] = useState("32");
  ...
  const [projectCutLongPauses, setProjectCutLongPauses] = useState(true);
  const [projectPauseThresholdMs, setProjectPauseThresholdMs] = useState("800");
  ...
  const fetchTaskStatus = useCallback(
    async (retryCount = 0, maxRetries = 5) => {
      ...
      try {
        ...
        if (!taskResponse.ok) {
          throw new Error(await buildSupportError(taskResponse, `Erro ao buscar a geração: ${taskResponse.status}`));
        }

        const taskData = await taskResponse.json();
        setTask(taskData);
        setProjectFontFamily(taskData.font_family || "THEBOLDFONT");
        setProjectFontSize(String(taskData.font_size || 32));
        ...
        setProjectPauseThresholdMs(String(taskData.pause_threshold_ms || 800));
        ...
        if (taskData.status === "completed" || taskData.status === "processing") {
          ...
          if (!clipsResponse.ok) {
            throw new Error(await buildSupportError(clipsResponse, `Erro ao buscar os cortes: ${clipsResponse.status}`));
          }
        ...
        setError(err instanceof Error ? err.message : "Não foi possível carregar a geração");
        return false;
      }
    },
  ...
  // Backend messages are already in pt-BR for new runs; this maps legacy
  // English progress strings from older tasks.
  const traduzirProgresso = (message: string) => {
    if (!message) return message;
    const mapa: Array<[RegExp, string]> = [
      [/^Starting\.\.\.$/, "Iniciando..."],
      [/^Downloading video\.\.\.$/, "Baixando o vídeo..."],
      [/^Generating transcript\.\.\.$/, "Transcrevendo o áudio..."],
      [/^Analyzing content with AI\.\.\.$/, "Analisando os melhores momentos..."],
      [/^Creating video clips\.\.\.$/, "Gerando os cortes..."],
      [/^Creating clip (\d+)\/(\d+)\.\.\.$/, "Gerando corte $1/$2..."],
      [/^Complete!$/, "Concluído!"],
      [/^Cancelled by user$/, "Cancelado pelo usuário"],
    ];
    for (const [pattern, replacement] of mapa) {
  ...
  const getHookTypeLabel = (hookType: string | null) => {
    const labels: Record<string, string> = {
      ...
      statistic: "Dados/Números",
      story: "Gancho de história",
      contrast: "Gancho de contraste",
      fallback: "Trecho inicial",
      none: "Sem gancho",
    };
    return labels[hookType || "none"] || hookType || "Nenhum";
  };
  ...
  const handleEditTitle = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(await buildSupportError(response, "Não foi possível atualizar o título"));
      }
    } catch (err) {
      console.error("Error updating title:", err);
      alert(err instanceof Error ? err.message : "Não foi possível atualizar o título");
    }
  };
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(await buildSupportError(response, "Não foi possível excluir a geração"));
      }
    } catch (err) {
      console.error("Error deleting task:", err);
      alert(err instanceof Error ? err.message : "Não foi possível excluir a geração");
    } finally {
      setIsDeleting(false);
  ...
  const handleDeleteClip = async (clipId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(await buildSupportError(response, "Não foi possível excluir o corte"));
      }
    } catch (err) {
      console.error("Error deleting clip:", err);
      alert(err instanceof Error ? err.message : "Não foi possível excluir o corte");
    }
  };
  ...
  const handleTrimClip = async (clipId: string) => {
    ...
    if (!response.ok) {
      alert(await buildSupportError(response, "Não foi possível aparar o corte"));
      return;
    }
    await fetchTaskStatus();
  };
  ...
  const handleSplitClip = async (clipId: string) => {
    ...
    if (!response.ok) {
      alert(await buildSupportError(response, "Não foi possível dividir o corte"));
      return;
    }
    await fetchTaskStatus();
  };
  ...
  const handleMergeClips = async () => {
    ...
    if (!response.ok) {
      alert(await buildSupportError(response, "Não foi possível unir os cortes"));
      return;
    }
    setSelectedClipIds([]);
    await fetchTaskStatus();
  ...
  const handleUpdateCaptions = async (clipId: string) => {
    ...
    if (!response.ok) {
      alert(await buildSupportError(response, "Não foi possível atualizar as legendas"));
      return;
    }
    await fetchTaskStatus();
  };
  ...
  const handleApplyProjectSettings = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        alert(await buildSupportError(response, "Não foi possível aplicar as configurações"));
        return;
      }
      await fetchTaskStatus();
    } finally {
  ...
  const handleExportClip = async (clipId: string, fallbackFilename: string) => {
    ...
    if (!response.ok) {
      alert(await buildSupportError(response, "Não foi possível exportar o corte"));
      return;
    }
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className="flex items-center gap-1 cursor-default">
                        ...
                        {new Date(task.created_at).toLocaleDateString("pt-BR", { year: "numeric", month: "short", day: "numeric" })}
                      </span>
                    </TooltipTrigger>
                    <TooltipContent>
                ...
                {task.status === "completed" ? (
                  <span>
                    {clips.length} {clips.length === 1 ? "corte gerado" : "cortes gerados"}
                  </span>
                ) : task.status === "processing" ? (
                  <div className="relative group">
                    <Badge className="bg-blue-100 text-blue-800 cursor-default shimmer">Processando</Badge>
                    <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md border bg-popover px-3 py-1.5 text-sm text-popover-foreground shadow-md opacity-0 scale-95 transition-all group-hover:opacity-100 group-hover:scale-100 pointer-events-none">
                      🔍&nbsp;&nbsp;Estamos processando seu vídeo. Volte em alguns minutos.
                    </div>
                  </div>
                ) : task.status === "queued" ? (
                  <Badge className="bg-yellow-100 text-yellow-800">Na fila</Badge>
                ) : task.status === "error" ? (
            ...
            <div className="flex flex-col items-center py-8">
              ...
              <div className="relative group flex items-center gap-1.5 mb-8 cursor-default">
                ...
                <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md border bg-popover px-3 py-1.5 text-sm text-popover-foreground shadow-md opacity-0 scale-95 transition-all group-hover:opacity-100 group-hover:scale-100 pointer-events-none">
                  ☕&nbsp;&nbsp;Pega um café — quando voltar, os cortes estarão prontos.
                </div>
              </div>

              {/* Status message */}
              <p className="shimmer text-neutral-600/60 text-sm tracking-wide mb-8">
                {traduzirProgresso(progressMessage) || (task.status === "queued" ? "Aguardando na fila" : "Processando")}
              </p>
            ...
            {clips.length > 0 && (
              <div className="grid gap-6">
                <p className="text-sm text-neutral-500 text-center">
                  {clips.length} {clips.length !== 1 ? "cortes prontos" : "corte pronto"}
                </p>
                {clips.map((clip) => (
                    ...
                    <CardContent className="p-0">
                      <div className="flex flex-col lg:flex-row">
                        ...
                        <div className="p-6 flex-1">
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              <h3 className="font-semibold text-lg text-black mb-1">
                                {clip.suggested_title || `Corte ${clip.clip_order}`}
                              </h3>
                            ...
                            <div className="flex items-center gap-2">
                              {(clip.overall_score ?? clip.virality_score) > 0 && (
                                <Badge className={`${getViralityBgColor(clip.overall_score ?? clip.virality_score)} text-white`}>
                                  <Zap className="w-3 h-3 mr-1" />
                                  {clip.overall_score ?? clip.virality_score}
                                </Badge>
                              )}
                          ...
                          {clip.text && (
                            <div className="mb-4">
                              <h4 className="font-medium text-black mb-2">Transcrição</h4>
          ...
          <Card>
            <CardContent className="p-8 text-center">
              <div className="text-red-600 mb-4">
                ...
                <h2 className="text-xl font-semibold">O processamento falhou</h2>
              </div>
              <p className="text-gray-600 mb-4">
                {task?.progress_message
                  ? traduzirProgresso(task.progress_message)
                  : "Ocorreu um erro ao processar o vídeo. Tente novamente."}
              </p>
              <Link href="/">
                <Button>
                  <ArrowLeft className="w-4 h-4" />
              ...
              {task?.status === "completed" ? (
                <>
                  <div className="text-yellow-600 mb-4">
                    <AlertCircle className="w-12 h-12 mx-auto mb-2" />
                    <h2 className="text-xl font-semibold">Nenhum corte gerado</h2>
                  </div>
                  <p className="text-gray-600 mb-4">
                    O processamento terminou, mas nenhum corte foi gerado. O vídeo pode não ter
                    trechos adequados para cortes.
                  </p>
                  <Link href="/">
                    <Button>
                      <ArrowLeft className="w-4 h-4" />
                ...
                <>
                  ...
                  <h2 className="text-xl font-semibold text-black mb-2">Ainda gerando...</h2>
                  <p className="text-gray-600">
                    Seus cortes estão sendo gerados. Esta página vai atualizar sozinha quando ficarem prontos.
                  </p>
                </>
              )}
            </CardContent>
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedClipIds.length >= 2 && (
                <Button variant="outline" size="sm" onClick={handleMergeClips}>
                  ...
                  Unir selecionados ({selectedClipIds.length})
                </Button>
              )}
            </div>
            ...
            <Sheet open={settingsSheetOpen} onOpenChange={setSettingsSheetOpen}>
              <SheetContent side="right" className="sm:max-w-md overflow-y-auto">
                <SheetHeader>
                  ...
                  <SheetDescription>
                    Ajuste fonte, legendas e B-roll para os cortes desta geração.
                  </SheetDescription>
                </SheetHeader>
                ...
                <div className="space-y-5 px-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-medium text-gray-500">Estilo de legenda</label>
                    <Select value={projectCaptionTemplate} onValueChange={setProjectCaptionTemplate}>
                      <SelectTrigger>
                        <SelectValue>
                          {availableTemplates.find((t) => t.id === projectCaptionTemplate)?.name || "Escolha o estilo"}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        ...
                        {availableTemplates.length === 0 && <SelectItem value="default">Padrão</SelectItem>}
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="rounded-lg border bg-gray-50 p-3 space-y-3">
                    <div>
                      <div className="text-sm font-medium text-gray-900">Limpeza dos cortes</div>
                      <div className="text-xs text-gray-500">Aplica corte de silêncios e vícios de fala ao regenerar os cortes.</div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-gray-500">Limite de pausa (ms)</label>
                      <Input
                        type="number"
                        min={250}
                        max={3000}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-gray-500">Palavras ou frases extras para remover</label>
                      <Input
                        value={projectFilteredWords}
                        onChange={(e) => setProjectFilteredWords(e.target.value)}
                        placeholder="basicamente, literalmente, na real"
                      />
                    </div>
                  </div>
                </div>
                ...
                <SheetFooter>
                  ...
                  >
                    {isApplyingSettings ? "Aplicando..." : "Aplicar a todos os cortes"}
                  </Button>
                </SheetFooter>
              </SheetContent>
            </Sheet>
                ...
                <CardContent className="p-0">
                  <div className="flex flex-col lg:flex-row">
                    ...
                    <div className="p-6 flex-1">
                      ...
                      {/* Post metadata: description + hashtags */}
                      {(clip.suggested_description || (clip.hashtags && clip.hashtags.length > 0)) && (
                        <div className="mb-4 p-3 bg-stone-50 border border-stone-200 rounded-lg space-y-2">
                          {clip.suggested_description ? (
                            <p className="text-sm text-gray-800">{clip.suggested_description}</p>
                          ) : null}
                      ...
                      {(clip.overall_score ?? clip.virality_score) > 0 && (
                        <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <span className={`text-lg font-bold ${getViralityColor(clip.overall_score ?? clip.virality_score)}`}>
                              {clip.overall_score ?? clip.virality_score}/100
                            </span>
                          </div>

                          {typeof clip.retention_score === "number" && (clip.overall_score ?? 0) > 0 ? (
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              {[
                                { rotulo: "Gancho", icone: <MessageSquare className="w-3 h-3" />, valor: clip.hook_score > 10 ? Math.round(clip.hook_score / 2.5) : clip.hook_score },
                                { rotulo: "Retenção", icone: <TrendingUp className="w-3 h-3" />, valor: clip.retention_score ?? 0 },
                                { rotulo: "Emoção", icone: <Zap className="w-3 h-3" />, valor: clip.emotion_score ?? 0 },
                                { rotulo: "Clareza", icone: <Subtitles className="w-3 h-3" />, valor: clip.clarity_score ?? 0 },
                                { rotulo: "Ritmo", icone: <Clock className="w-3 h-3" />, valor: clip.pacing_score ?? 0 },
                                { rotulo: "Desfecho", icone: <Star className="w-3 h-3" />, valor: clip.payoff_score ?? 0 },
                                { rotulo: "Loop", icone: <RefreshCw className="w-3 h-3" />, valor: clip.loop_score ?? 0 },
                                { rotulo: "Contexto próprio", icone: <Share2 className="w-3 h-3" />, valor: clip.standalone_context_score ?? 0 },
                              ].map((item) => (
                                <div key={item.rotulo} className="space-y-1">
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-medium">{item.valor}/10</span>
                                  </div>
                                  <Progress value={(item.valor / 10) * 100} className="h-1.5" />
                                </div>
                              ))}
                            </div>
                          ) : (
                          ...
                          <div className="mt-3 pt-2 border-t flex flex-wrap items-center gap-2">
                            ...
                            {clip.reasoning ? (
                              <span className="text-xs text-gray-600">{clip.reasoning}</span>
                            ) : null}
                          </div>
                        </div>
                      )}
                        ...
                        <div className="mt-4 p-3 border rounded-lg space-y-3 bg-gray-50">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                            <Input
                              ...
                              placeholder="Aparar início (s)"
                            />
                            <Input
                              value={endOffset}
                              onChange={(e) => setEndOffset(e.target.value)}
                              placeholder="Aparar fim (s)"
                            />
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                            ...
                            <Select value={captionPosition} onValueChange={setCaptionPosition}>
                              <SelectTrigger>
                                <SelectValue placeholder="Posição da legenda" />
                              </SelectTrigger>
                              <SelectContent>
                            ...
                            <Input
                              ...
                              placeholder="Destaques: palavra1, palavra2"
                            />
                          </div>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza de que quer excluir esta geração? Todos os cortes serão apagados
              permanentemente e não será possível desfazer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={!!deletingClipId} onOpenChange={(open) => !open && setDeletingClipId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza de que quer excluir este corte? Esta ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
