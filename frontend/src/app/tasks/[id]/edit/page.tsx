...
export default function TaskEditPage() {
  ...
  const fetchEditorData = useCallback(async () => {
    ...
    try {
      ...
      setError(fetchError instanceof Error ? fetchError.message : "Não foi possível carregar o editor");
    }
  ...
  const handleTrim = async () => {
    ...
    await withSaving(async () => {
      ...
      if (!response.ok) throw new Error(await buildSupportError(response, "Não foi possível aparar o corte"));
    });
  };
  ...
  const handleSplit = async (splitAt?: number) => {
    ...
    await withSaving(async () => {
      ...
      if (!response.ok) throw new Error(await buildSupportError(response, "Não foi possível dividir o corte"));
    });
  };
  ...
  const handleUpdateCaptions = async () => {
    ...
    await withSaving(async () => {
      ...
      if (!response.ok) throw new Error(await buildSupportError(response, "Não foi possível atualizar as legendas"));
    });
  };

  const handleMerge = async () => {
    if (!task?.id || mergeSelection.length < 2) return;
    await withSaving(async () => {
      ...
      if (!response.ok) throw new Error(await buildSupportError(response, "Não foi possível unir os cortes selecionados"));
    });
    setMergeSelection([]);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="border-b bg-white">
        <div className="max-w-7xl mx-auto px-4 py-5 flex items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl font-bold text-black">{task?.source_title || "Editor de cortes"}</h1>
          </div>
          <Button onClick={handleExport} disabled={!selectedClip || isSaving}>
            <Download className="w-4 h-4" />
            {exportProgress !== null ? `Exportando ${exportProgress}%` : "Exportar selecionado"}
          </Button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {!task ? (
          <Alert>
            <AlertDescription>Geração não encontrada.</AlertDescription>
          </Alert>
        ) : task.status !== "completed" ? (
          <Card>
            <CardContent className="p-8 text-center space-y-3">
              <p className="text-lg font-semibold">O editor fica disponível quando o processamento termina.</p>
              <p className="text-gray-600">Status atual: {task.status}</p>
              <Link href={`/tasks/${task.id}`}>
                <Button variant="outline">Voltar à geração</Button>
              </Link>
            </CardContent>
          </Card>
        ) : clips.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center space-y-3">
              <p className="text-lg font-semibold">Nenhum corte para editar ainda.</p>
              <Link href={`/tasks/${task.id}`}>
                <Button variant="outline">Voltar à geração</Button>
              </Link>
            </CardContent>
                    ...
                    <>
                      ...
                      <div className="border rounded-lg p-3 space-y-3">
                        <div className="flex items-center justify-between text-sm text-gray-600">
                          <span>Posição: {formatDuration(currentTime)} / {formatDuration(selectedClip.duration)}</span>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                          ...
                          <Button variant="outline" size="sm" onClick={setTrimInToPlayhead}>Marcar início</Button>
                          <Button variant="outline" size="sm" onClick={setTrimOutToPlayhead}>Marcar fim</Button>
                        </div>
                      </div>

                      <div className="border rounded-lg p-3 space-y-3">
                        <div className="flex items-center justify-between text-sm text-gray-700">
                          <span className="font-medium">Intervalo do corte</span>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                          ...
                          <Button variant="outline" onClick={() => seekTo(trimRange[0])}>Ir ao início</Button>
                          <Button variant="outline" onClick={() => seekTo(trimRange[1])}>Ir ao fim</Button>
                        </div>
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-gray-600">Selecione um corte para começar a editar.</p>
                  )}
                </CardContent>
              </Card>
                  ...
                  <CardContent className="space-y-5">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-sm">
                        <span className="flex items-center gap-2"><SplitSquareVertical className="w-4 h-4" />Dividir</span>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Button variant="outline" onClick={() => setSplitTime(currentTime)} disabled={!selectedClip}>Usar posição atual</Button>
                        <Button variant="outline" onClick={() => void handleSplit()} disabled={isSaving || !selectedClip}>Dividir corte</Button>
                      </div>
                    </div>

                    <div className="space-y-3">
                      <div className="text-sm font-medium flex items-center gap-2"><AudioLines className="w-4 h-4" />Áudio</div>
                    ...
                    <div className="space-y-3">
                      <div className="text-sm font-medium flex items-center gap-2"><Palette className="w-4 h-4" />Efeitos de vídeo</div>
                      {[
                        ["Brilho", "brightness", 40, 180, 1],
                        ["Contraste", "contrast", 40, 180, 1],
                        ["Saturação", "saturation", 0, 220, 1],
                        ["Desfoque", "blur", 0, 8, 0.1],
                        ["Matiz", "hue", -180, 180, 1],
                        ["Zoom", "zoom", 1, 2, 0.01],
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <textarea
                      ...
                      placeholder="Edite o texto da legenda"
                    ...
                    <div className="space-y-2">
                      <div className="text-xs text-gray-600">Palavras em destaque (clique para alternar)</div>
                      <div className="max-h-28 overflow-y-auto rounded-md border border-gray-200 p-2 flex flex-wrap gap-1.5">
                        {subtitleWords.length === 0 ? (
                          <span className="text-xs text-gray-500">Nenhuma palavra ainda.</span>
                        ) : (
                ...
                {mergeSelection.length >= 2 && (
                  <div className="flex justify-end">
                    <Button variant="outline" onClick={handleMerge} disabled={isSaving}>
                      Unir selecionados ({mergeSelection.length})
                    </Button>
                  </div>
                )}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="font-medium text-sm text-black">Corte {clip.clip_order}</p>
