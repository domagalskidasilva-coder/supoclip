...
function sourceBadge(setting: RuntimeSetting) {
  if (setting.source === "environment") {
    return <Badge className="bg-black text-white">Arquivo .env</Badge>;
  }
  if (setting.source === "admin") {
    return <Badge className="bg-blue-100 text-blue-800">Valor salvo</Badge>;
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(payload?.detail || payload?.error || "Não foi possível salvar as configurações");
        return;
      }
    ...
    <form onSubmit={handleSubmit}>
          ...
          >
            ...
            <div>
              <input
                ...
                placeholder={
                  setting.configured ? "Valor configurado (oculto)" : "Adicionar valor"
                }
              ...
              {setting.overridden_by_env && (
                <p className="mt-1 text-xs text-amber-700">
                  Existe um valor salvo, mas ele é ignorado enquanto a variável do .env estiver definida.
                </p>
              )}
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 px-4 py-4">
        ...
        >
          ...
          {isPending || isSaving ? "Salvando" : "Salvar configurações"}
        </button>
      </div>
    </form>
  );
