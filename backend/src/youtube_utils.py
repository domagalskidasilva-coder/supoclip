...
class YouTubeDownloader:
    ...
    def get_optimal_download_options(
        ...
        opts = {
            ...
            "ignoreerrors": False,
            # NOTE: do not override http_headers/User-Agent here. Spoofed
            # browser headers conflict with yt-dlp's per-client impersonation
            # and its JS-challenge solving, triggering YouTube bot checks.
            # Metadata extraction
            "extract_flat": False,
            "writeinfojson": False,
...
def _build_info_options() -> Dict[str, Any]:
    ydl_opts = {
        ...
        "skip_download": True,
        "socket_timeout": 30,
        # NOTE: no custom http_headers — see get_optimal_download_options.
        "nocheckcertificate": True,
    }
    return ydl_opts
...
def _download_youtube_video_with_ytdlp(
    ...
    if not video_info:
        # Metadata-only lookups hit YouTube's bot-check independently of the
        # actual download (different client/format negotiation), so a failed
        # pre-flight here does not mean the download itself would fail too.
        # Proceed to the real download attempts instead of aborting; duration
        # is still enforced post-download in video_service.py.
        logger.warning(
            "Could not retrieve video metadata for %s; proceeding to download anyway",
            url,
        )
    else:
