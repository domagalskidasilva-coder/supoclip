...
def _normalize_font_size(value: Any, default: int = 32) -> int:
    # 32 matches the caption templates' base size: large, readable captions
    # by default (the renderer scales this with the output width).
    try:
        parsed = int(value)
...
def _normalize_clip_duration(value: Any, default: int | None = None) -> int | None:
    if value is None:
        return default
    try:
        parsed = int(value)
...
def _merge_task_source_metadata(
    ...
    *,
    ...
    normalized_clip_duration = _normalize_clip_duration(clip_duration)
    if normalized_clip_duration is not None:
        merged["clip_duration"] = normalized_clip_duration
...
async def _require_task(
    task_service: TaskService, db: AsyncSession, task_id: str
):
    """Load a task or raise 404."""
...
async def list_tasks(
    ...
    try:
        ...
        tasks = await task_service.get_tasks(limit)
...
async def create_task(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    font_size = _normalize_font_size(font_options.get("font_size", 32))
    ...
    if not isinstance(add_subtitles, bool):
        add_subtitles = True
    clip_duration = _normalize_clip_duration(
        data.get("clip_duration"), runtime_config.clip_duration
    )
...
async def get_billing_summary():
    """Compatibility response for clients that still ask for billing status."""
    return {
...
async def get_task(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        task_service = TaskService(db)
        await _require_task(task_service, db, task_id)
...
async def update_task(
    ...
    try:
        ...
        task = await _require_task(task_service, db, task_id)
...
async def delete_clip(
    task_id: str, clip_id: str, db: AsyncSession = Depends(get_db)
):
...
async def apply_task_settings(
    ...
    try:
        ...
        font_size = _normalize_font_size(payload.get("font_size", 32))
        ...
        if not task_record:
            raise HTTPException(status_code=404, detail="Task not found")
        if not is_font_accessible(font_family):
            raise HTTPException(
...
async def resume_task(
    ...
    try:
        ...
        if isinstance(asub, bool):
            add_subtitles = asub
        clip_duration = _normalize_clip_duration(
            metadata.get("clip_duration"), runtime_config.clip_duration
        )
        cleanup_settings = normalize_clip_cleanup_settings(
