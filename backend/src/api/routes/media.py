...
from ...font_registry import (
    FONTS_DIR,
    GLOBAL_CUSTOM_FONTS_DIR,
    SUPPORTED_FONT_EXTENSIONS,
    build_global_font_stem,
    find_font_path,
...
async def upload_font(
    ...
    try:
        ...
        GLOBAL_CUSTOM_FONTS_DIR.mkdir(parents=True, exist_ok=True)

        original_stem = sanitize_font_stem(uploaded_filename)
        stored_stem = build_global_font_stem(original_stem)
        target_path = GLOBAL_CUSTOM_FONTS_DIR / f"{stored_stem}{extension}"
        suffix = 2
        while target_path.exists():
            target_path = GLOBAL_CUSTOM_FONTS_DIR / f"{stored_stem}-{suffix}{extension}"
            suffix += 1
