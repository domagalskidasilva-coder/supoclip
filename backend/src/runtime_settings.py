...
RUNTIME_SETTING_KEYS: tuple[str, ...] = (
    ...
    "OPENAI_API_KEY",
    "GOOGLE_API_KEY",
    # Encrypted JSON array of up to 5 Google keys used as an automatic
    # failover pool by the AI analysis step. Managed by dedicated
    # /admin/google-api-keys endpoints, not the generic settings form.
    "GOOGLE_API_KEYS",
    "ANTHROPIC_API_KEY",
    "OLLAMA_BASE_URL",
    "OLLAMA_API_KEY",
...
def _get_encryption_secret() -> str:
    secret = os.getenv("APP_SETTINGS_ENCRYPTION_KEY")
    if not secret or len(secret.strip()) < 16:
        raise RuntimeError(
            "APP_SETTINGS_ENCRYPTION_KEY must be set to save encrypted runtime settings."
        )
    return secret.strip()
