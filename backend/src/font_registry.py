...
GLOBAL_CUSTOM_FONTS_DIR = FONTS_DIR / "custom"
...
def get_available_fonts() -> list[dict[str, Any]]:
    ...
    fonts.extend(_collect_fonts_from_dir(GLOBAL_CUSTOM_FONTS_DIR, scope="custom"))
...
def find_font_path(
    ...
    for search_dir in [GLOBAL_CUSTOM_FONTS_DIR, FONTS_DIR]:
...
def build_global_font_stem(original_stem: str) -> str:
    safe_stem = sanitize_font_stem(original_stem)
    return f"custom-{safe_stem}".lower()


def is_font_accessible(font_name: str) -> bool:
