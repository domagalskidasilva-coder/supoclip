...
def _assemblyai_speech_models(speech_model: str) -> list[str]:
    normalized_models = [
        item.strip().lower()
        for item in (speech_model or "universal-3-pro,universal-2").split(",")
        if item.strip()
    ]
    ...
    for model in normalized_models:
        if model in {"best", "universal", "universal-2", "universal-3-pro"}:
            models.extend(["universal-3-pro", "universal-2"])
        elif model in {"nano"}:
            models.append("universal-2")
        elif model in {"slam-1", "slam_1"}:
            models.append("slam-1")
        else:
            models.append(model)
...
def get_video_transcript(video_path: Path, speech_model: str = "universal-3-pro,universal-2") -> str:
...
def ffmpeg_escape_filter_path(path: Path) -> str:
    ...
    Windows paths (e.g. ``C:\\Users\\...``) go through libavfilter's
    option-string parser twice for filters like ``subtitles`` (once for the
    outer ``-vf`` filtergraph split, once for the filter's own ``key=value``
    sub-option split). Doubling backslashes there still leaves the drive
    letter's colon ambiguous and ffmpeg fails with "No option name" /
    "Invalid argument". ffmpeg accepts forward slashes as path separators on
    Windows too, so normalizing to forward slashes first sidesteps backslash
    escaping entirely and only the drive-letter colon needs escaping.
    """
...
def ffmpeg_escape_filter_value(value: str) -> str:
    """Escape an ffmpeg filter option value (see ffmpeg_escape_filter_path)."""
...
def build_clip_keep_ranges(
    ...
    if cut_long_pauses:
        # Leading dead air is pure loss for retention: cut it with a much
        # tighter threshold than mid-clip pauses so clips open directly on
        # the first spoken word (a ~0.15s breath is kept so the cut does not
        # clip the attack of the first syllable).
        leading_silence_threshold = min(0.35, pause_threshold_seconds)
        leading_gap = relevant_words[0]["start"] - clip_start
        if leading_gap >= leading_silence_threshold:
            removal_intervals.append(
                (clip_start, max(clip_start, relevant_words[0]["start"] - 0.15))
            )
        ...
        try:
            ...
            if success:
                ...
                clip_info = {
                    ...
                    # Viral scorecard + post metadata pass-through
