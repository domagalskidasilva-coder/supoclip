...
class VideoService:
    ...
    async def create_single_clip(
        ...
        try:
            ...
            if not success:
                fallback_ranges = extend_keep_ranges_to_sentence_boundary(
                    video_path,
                    raw_keep_ranges,
                )
                if fallback_ranges and fallback_ranges != keep_ranges:
                    logger.warning(
                        "Retrying clip %s without automatic cleanup",
                        clip_index + 1,
                    )
                    success = await run_in_thread(
                        create_optimized_clip,
            ...
            if not success and add_subtitles:
                fallback_ranges = extend_keep_ranges_to_sentence_boundary(
                    video_path,
                    raw_keep_ranges,
                )
                if fallback_ranges:
                    logger.warning(
                        "Retrying clip %s without subtitles",
                        clip_index + 1,
                    )
                    success = await run_in_thread(
                        create_optimized_clip,
            ...
            return {
                ...
                "emotion_score": segment.get("emotion_score", 0),
                "clarity_score": segment.get("clarity_score", 0),
                "pacing_score": segment.get("pacing_score", 0),
                "payoff_score": segment.get("payoff_score", 0),
                "loop_score": segment.get("loop_score", 0),
                "standalone_context_score": segment.get("standalone_context_score", 0),
                "overall_score": segment.get(
                    "overall_score", segment.get("virality_score", 0)
                ),
                "suggested_title": segment.get("suggested_title", ""),
                "suggested_description": segment.get("suggested_description", ""),
    ...
    async def process_video_complete(
        ...
        try:
            ...
            try:
                ...
                target_clip_duration = int(runtime_config.clip_duration)
            target_clip_duration = max(15, min(90, target_clip_duration))
            ...
            if progress_callback:
                await progress_callback(10, "Baixando o vídeo...", "processing")
            ...
            if progress_callback:
                await progress_callback(30, "Transcrevendo o áudio...", "processing")
            ...
            if progress_callback:
                await progress_callback(
                    50, "Analisando os melhores momentos...", "processing"
                )
            ...
            if progress_callback:
                await progress_callback(70, "Gerando os cortes...", "processing")

            raw_segments = relevant_parts.most_relevant_segments
            segments_json: List[Dict[str, Any]] = []
            for segment in raw_segments:
                if isinstance(segment, dict):
                    # Cached analysis payloads are already flattened by a
                    # previous run of this mapping.
                    segments_json.append(
                        {
                            ...
                            "payoff_score": segment.get("payoff_score", 0),
                            "loop_score": segment.get("loop_score", 0),
                            "standalone_context_score": segment.get(
                                "standalone_context_score", 0
                            ),
                            "overall_score": segment.get(
                                "overall_score", segment.get("virality_score", 0)
                            ...
                            "suggested_title": segment.get("suggested_title", ""),
                            "suggested_description": segment.get(
                                "suggested_description", ""
                            ),
                            "hashtags": segment.get("hashtags", []),
                        }
                    ...
                    scorecard = segment.virality
                    legacy = (
                        scorecard.legacy_virality_fields()
                    ...
                    segments_json.append(
                        {
                            ...
                            "virality_score": legacy.get("virality_score", 0),
                            "hook_score": legacy.get("hook_score", 0),
                            "engagement_score": legacy.get("engagement_score", 0),
                            "value_score": legacy.get("value_score", 0),
                            "shareability_score": legacy.get("shareability_score", 0),
                            "hook_type": scorecard.hook_type if scorecard else None,
                            "retention_score": scorecard.retention_score if scorecard else 0,
                            "emotion_score": scorecard.emotion_score if scorecard else 0,
                            "clarity_score": scorecard.clarity_score if scorecard else 0,
                            "pacing_score": scorecard.pacing_score if scorecard else 0,
                            "payoff_score": scorecard.payoff_score if scorecard else 0,
                            "loop_score": scorecard.loop_score if scorecard else 0,
                            "standalone_context_score": (
                                scorecard.standalone_context_score if scorecard else 0
                            ),
                            "overall_score": scorecard.overall_score if scorecard else 0,
                            "suggested_title": segment.suggested_title,
                            "suggested_description": segment.suggested_description,
                            "hashtags": list(segment.hashtags or []),
                        }
                    )
