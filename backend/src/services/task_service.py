...
class TaskService:
    ...
    def _build_cache_key(
        ...
        duration_part = (
            f"|duration={clip_duration}" if clip_duration is not None else ""
        )
        payload = (
            f"{source_type}|{processing_mode}|"
            f"{TRANSCRIPT_ANALYSIS_CACHE_VERSION}{duration_part}|{url.strip()}"
        )
        return hashlib.sha256(payload.encode("utf-8")).hexdigest()

    @staticmethod
    def _normalize_clip_duration(value: Any) -> int | None:
        if value is None:
            return None
        try:
            parsed = int(value)
    ...
    async def _queued_task_has_pending_job(self, task_id: str) -> bool:
        """Return True when Redis still has an ARQ job queued for this task."""
        ...
        try:
            job_ids = await redis_client.zrange(DEFAULT_QUEUE_NAME, 0, -1)
            task_marker = task_id.encode("utf-8")
            for job_id in job_ids or []:
                job_key = b"arq:job:" + bytes(job_id)
                payload = await redis_client.get(job_key)
                if payload and task_marker in payload:
                    return True
            return False
        except Exception as exc:
            logger.warning("Failed to inspect Redis queue for task %s: %s", task_id, exc)
            return False
        finally:
    ...
    async def process_task(
        ...
        try:
            ...
            normalized_clip_duration = self._normalize_clip_duration(clip_duration)
            cache_key = self._build_cache_key(
                url, source_type, processing_mode, normalized_clip_duration
            )
            ...
            result = await self.video_service.process_video_complete(
                ...
                clip_duration=normalized_clip_duration,
            ...
            def build_fallback_segment() -> Dict[str, Any]:
                fallback_duration = (
                    result.get("clip_duration")
                    or normalized_clip_duration
                    or self.config.clip_duration
                )
                return self.video_service._build_fallback_segment(
                    self.video_service._get_file_duration(video_path),
                    result.get("transcript") or "",
                    int(fallback_duration),
                )

            segments_to_render = result.get("segments_to_render") or []
            if not segments_to_render:
                logger.warning(
                    "No usable clip segments were selected for task %s; "
                    "rendering a fallback clip",
                    task_id,
                )
                fallback_segment = build_fallback_segment()
                segments_to_render = [fallback_segment]
                result["segments_to_render"] = segments_to_render
                result["segments"] = segments_to_render
                result["analysis_json"] = json.dumps(
                    {
                        "summary": result.get("summary"),
                        "key_topics": result.get("key_topics") or [],
                        "most_relevant_segments": segments_to_render,
                    }
                )
            ...
            async def persist_clip(clip_info: Dict[str, Any], clip_order: int) -> str:
                clip_id = await self.clip_repo.create_clip(
                    ...
                    scorecard=self._extract_scorecard(clip_info),
                )
            ...
            for i, segment in enumerate(segments_to_render):
                ...
                await update_progress(
                    clip_progress,
                    f"Gerando corte {i + 1}/{total_clips}...",
                )
                ...
                clip_id = await persist_clip(clip_info, i + 1)
            ...
            if not clip_ids:
                logger.warning(
                    "All selected segments failed to render for task %s; "
                    "rendering the first available portion of the video",
                    task_id,
                )
                await update_progress(95, "Gerando corte inicial...", "processing")
                fallback_segment = build_fallback_segment()
                fallback_cleanup_settings = normalize_clip_cleanup_settings(
                ...
                clip_info = await self.video_service.create_single_clip(
                    ...
                    output_format,
                    add_subtitles,
                    fallback_cleanup_settings,
                )
                if clip_info is None and add_subtitles:
                    clip_info = await self.video_service.create_single_clip(
                        video_path,
                        fallback_segment,
                        0,
                ...
                if clip_info is None:
                    raise ValueError(
                        "Não foi possível renderizar nenhum corte para este vídeo."
                    )

                result["segments"] = [fallback_segment]
                clip_id = await persist_clip(clip_info, 1)
                if clip_ready_callback:
                    clip_record = await self.clip_repo.get_clip_by_id(
                        self.db, clip_id
                    )
                    if clip_record:
                        await clip_ready_callback(0, 1, clip_record)
            ...
            if progress_callback:
                await progress_callback(100, "Concluído!", "completed")
            ...
            if str(e) == "Task cancelled":
                await self.task_repo.update_task_status(
                    ...
                    "cancelled",
                    progress=0,
                    progress_message="Cancelado pelo usuário",
                )
                raise
            await self.task_repo.update_task_status(
    ...
    async def _send_completion_notification_if_needed(
        ...
        logger.debug(
            "Skipping completion notification for global task %s (%s clips)",
            task_id,
            clips_count,
        )
    ...
    async def get_task_with_clips(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        if self._is_stale_queued_task(task):
            ...
            if await self._queued_task_has_pending_job(task_id):
                logger.warning(
                    f"Task {task_id} has been queued for over {timeout_seconds}s "
                    "but its ARQ job is still pending; keeping it queued"
                )
                await self.task_repo.update_task_status(
                    self.db,
                    task_id,
                    ...
                    progress=0,
                    progress_message=(
                        "Aguardando worker disponível. Confira se o scripts/dev-worker.ps1 está rodando."
                    ),
                )
                task = await self.task_repo.get_task_by_id(self.db, task_id)
                if not task:
                ...
                await self.task_repo.update_task_status(
                    ...
                    progress=0,
                    progress_message=(
                        "A geração expirou esperando na fila. "
                        "Confira se o scripts/dev-worker.ps1 está rodando e se o Redis está acessível."
                    ),
                )
                task = await self.task_repo.get_task_by_id(self.db, task_id)
                if not task:
    ...
    async def get_tasks(self, limit: int = 50) -> list[Dict[str, Any]]:
        """Get all tasks."""
        return await self.task_repo.get_tasks(self.db, limit)
    ...
    @staticmethod
    def _extract_scorecard(clip_info: Dict[str, Any]) -> Dict[str, Any]:
        """Collect viral scorecard fields + post metadata from a clip_info dict."""
        return {
            "retention_score": clip_info.get("retention_score", 0),
            "emotion_score": clip_info.get("emotion_score", 0),
            "clarity_score": clip_info.get("clarity_score", 0),
            "pacing_score": clip_info.get("pacing_score", 0),
            "payoff_score": clip_info.get("payoff_score", 0),
            "loop_score": clip_info.get("loop_score", 0),
            "standalone_context_score": clip_info.get("standalone_context_score", 0),
            "overall_score": clip_info.get(
                "overall_score", clip_info.get("virality_score", 0)
            ),
            "suggested_title": clip_info.get("suggested_title", ""),
            "suggested_description": clip_info.get("suggested_description", ""),
