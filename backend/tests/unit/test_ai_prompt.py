...
def test_build_transcript_analysis_prompt_uses_requested_clip_duration():
    prompt = build_transcript_analysis_prompt(
        transcript="[00:12 - 01:21] A strong opening line with payoff",
        clip_duration=60,
    )

    assert "Desired clip duration: about 60 seconds." in prompt
    assert "Prefer natural standalone ranges around 48-72 seconds." in prompt
    assert "Use 39-81 seconds when natural content boundaries require it." in prompt
    assert "Do not return segments shorter than 39 seconds or longer than 81 seconds." in prompt
...
def test_transcript_span_repair_respects_requested_duration():
    spans = _parse_transcript_spans(
        "\n".join(
            [
                "[00:00 - 00:20] Setup",
                "[00:20 - 00:40] Highlight",
                "[00:40 - 01:00] Payoff",
                "[01:00 - 01:20] Followup",
            ]
        )
    )

    assert _choose_repaired_bounds(spans, 0, 20, clip_duration=60) == (0, 60)
