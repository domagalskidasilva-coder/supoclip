...
def test_burn_ass_subtitles_passes_selected_fonts_dir(tmp_path):
    ...
    assert success is True
    video_filter = commands[0][commands[0].index("-vf") + 1]
    expected_fonts_dir = video_utils.ffmpeg_escape_filter_value(str(tmp_path / "fonts"))
    assert f"fontsdir={expected_fonts_dir}" in video_filter
    assert video_filter.endswith(",setsar=1")


def test_subtitles_filter_fragment_double_escapes_windows_drive_colons():
    fragment = video_utils.subtitles_filter_fragment(
        Path("C:/Users/User/AppData/Local/Temp/captions.ass"),
        Path("D:/supoclip-main/backend/fonts"),
    )

    assert r"filename=C\\:/Users/User/AppData/Local/Temp/captions.ass" in fragment
    assert r"fontsdir=D\\:/supoclip-main/backend/fonts" in fragment
