...
@pytest.mark.asyncio
async def test_create_single_clip_retries_without_cleanup_when_render_fails(
    monkeypatch, tmp_path
):
    render_keep_ranges = []

    async def fake_run_in_thread(_fn, *_args, **_kwargs):
        render_keep_ranges.append(_args[-1])
        return len(render_keep_ranges) == 2
