...
async def test_process_video_complete_uses_fallback_when_ai_selects_no_segments(
    ...
    async def fake_analyze_transcript(_transcript, clip_signals=None, clip_duration=None):
        assert clip_duration == 30
    ...
    assert len(result["segments_to_render"]) == 1
    fallback_segment = result["segments_to_render"][0]
    assert fallback_segment["start_time"] == "00:00"
    assert fallback_segment["end_time"] == "00:30"
    assert fallback_segment["text"] == "[00:00 - 00:01] hello"
    assert fallback_segment["relevance_score"] == 0.25
    assert fallback_segment["reasoning"] == (
    ...
    assert fallback_segment["overall_score"] == 0
