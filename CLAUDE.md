...
Services: Frontend (:3107), Backend API (127.0.0.1:8000, docs at /docs), Worker (ARQ), MCP (127.0.0.1:9100), PostgreSQL (internal only), Redis (host port :6380 → container :6379)
...
4. **Clip Generation** → ffmpeg/OpenCV (`video_utils.py`) creates 9:16 clips with:
...
**Task lifecycle:**
- `POST /tasks/` — Create task, enqueue to worker (returns task_id immediately)
